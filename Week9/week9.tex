...
\begin{frame}
  ...
  \begin{block}{}
    ...
    \alert{QUIZ:} Describe the {\bf transition} and the {\bf state table}
  \end{block}
  }
\end{frame}
...
\begin{frame}
  ...
  {\smaller
    ...
    \begin{block}{Better Recurrence -- $O(H*T)$}
      ...
      \item $\text{acorn}[j][i] += \text{max}(\text{acorn}[j][i-1], \text{maxac}[i-f])$
        \medskip
...
\begin{frame}
  ...
  {\smaller
    ...
    Assuming that we have a ``fast'' algorithm to test if $R$ can
    pass ($T(R)$), we could use \structure{Binary Search} to find the maximum
    R:
